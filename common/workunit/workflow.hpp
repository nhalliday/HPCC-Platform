...
protected:
    ...
    bool handleFailureParallel(CCloneWorkflowItem & item, WorkflowException const * e, bool isDep);
    //Returms true if a failure contingency has been queued
    bool activateFailureContingency(CCloneWorkflowItem & item, bool isAborting);
...
protected:
    ...
    //flag is set when a workflowItem fails
    std::atomic<bool> abort{false};
    //This protects against a race condition between global and item-specific abort flags
    //This also protects the consequent activation and deactivation of items
    CriticalSection abortCritSec;
    //This protects each item from having its exception set twice, in a race condition
    CriticalSection exceptionCritSec;
    //This protects each item from being activated twice, in a race condition
    ...
    //The number of branches (when reportFailureToFirstDependent = false) is the number of dependent successors to the failed workflow item.
    ...
    bool parallel = false;
    //Only one successor of a failed item gets notified. If false, all successors get notified.
    bool reportFailureToFirstDependant = true;
