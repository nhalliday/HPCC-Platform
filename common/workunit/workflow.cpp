...
public:
    ...
    {
        if(logicalSuccessors.contains(wfid))
            return true;
        else
            return false;
    }
...
#endif
                CCloneWorkflowItem * startItem = new CCloneWorkflowItem();
                markDependencies(parentWfid, startItem, false);
#ifdef TRACE_WORKFLOW
...
CCloneWorkflowItem * WorkflowMachine::insertLogicalPredecessor(unsigned successorWfid)
{
    ...
    markDependencies(successorWfid, predecessor, false);
    return predecessor;
}
//logicalPredecessor is for Sequential/Condition/Contingency
void WorkflowMachine::markDependencies(unsigned int wfid, CCloneWorkflowItem *logicalPredecessor, bool prevOrdered)
{
#ifdef TRACE_WORKFLOW
    ...
    if(item.queryMode() != WFModeCondition)
    {
        ...
        for(iter->first(); iter->isValid(); iter->next())
        {
            ...
            markDependencies(cur.queryWfid(), logicalPredecessor, thisOrdered);
            if(onlyProcessFirst)
                return;
            if(!alreadyProcessed)
    ...
    else
    {
        ...
        markDependencies(conditionExpression.queryWfid(), logicalPredecessor, false);
        if(alreadyProcessed)
            return;
        if(!iter->next())
            throwUnexpected();
    ...
    if(failureWfid)
    {
        //creates an intermediary item, and inserts it between "item" and the failure contingency
    ...
    for(iter->first();iter->isValid(); iter->next())
    {
        ...
        else if(isLogical)
        {
            if(!cur.queryContingencyWithin())
                cur.setContingencyWithin(item.queryContingencyWithin());//item may now be "active"
...
void WorkflowMachine::checkAbort(CCloneWorkflowItem & item)
{
...
void WorkflowMachine::executeItemParallel(unsigned wfid)
{
    ...
    CCloneWorkflowItem & item = queryWorkflowItem(wfid);
    {
        //item is deactivated if it is no longer alive
        CriticalBlock thisBlock(activationCritSec);
    ...
    if(item.queryException())
    {
        ...
        //else: process logical/dependent successors (further down)
    }
    ...
    if(!done)
    {
        //always process logical successors
        processLogicalSuccessors(item);
        bool alive;
        //NOTE - it won't make an item active/inactive directly, so it doesn't need to be protected by the activationCritSec
        {
